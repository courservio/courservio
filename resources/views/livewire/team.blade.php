    ...
    <div class="px-4 sm:px-6 lg:px-8">
        ...
        <div class="-mx-4 mt-8 overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:-mx-6 md:mx-0 md:rounded-lg">
            <table class="min-w-full divide-y divide-gray-300">
                ...
                <tr>
                    <th scope="col"
                        class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-gray-900 sm:pl-6">{{ _i('team name') }}</th>
                    <th scope="col"
                        class="px-3 py-3.5 text-left text-sm font-semibold text-gray-900">{{ _i('Display Name') }}</th>
                    <th scope="col"
                ...
                @foreach($teams as $team)
                    <tr>
                        ...
                        <td class="py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            @can('update', $team)
                                <x-button.link wire:click="edit({{ $team->id }})"><i
                ...
                <div class="space-y-3">
                    <div>
                        ...
                        <div class="relative mt-1 rounded-md shadow-sm">
                            <input type="text" wire:model.lazy="editing.name" name="{{ _i('team name') }}" id="name"
                                   class="@error('editing.name') border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:outline-none focus:ring-red-500 @enderror block w-full rounded-md border-gray-300 pr-10 lowercase focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                   placeholder="{{ _i('team name') }}" @error('editing.name') aria-invalid="true"
                    ...
                    <div>
                        <label for="display_name"
                               class="block text-sm font-medium text-gray-700">{{ _i('Display Team Name') }}</label>
                        <div class="relative mt-1 rounded-md shadow-sm">
                            <input type="text" wire:model.lazy="editing.display_name" name="display_name"
                                   id="display_name"
                                   class="@error('editing.display_name') border-red-300 text-red-900 placeholder-red-300 focus:border-red-500 focus:outline-none focus:ring-red-500 @enderror block w-full rounded-md border-gray-300 pr-10 focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm"
                                   placeholder="{{ _i('Display Team Name') }}"
                                   @error('editing.display_name') aria-invalid="true"
                                   aria-describedby="display_name-error" @enderror>
                        ...
                        <p class="mt-2 text-sm text-red-600"
                           id="name-error">{{ $errors->first('editing.display_name') }}</p>
                        @else
                            <p class="mt-2 text-xs text-gray-500"
                               id="name-description">{{ _i('The displayed team name.') }}</p>
                            @enderror
                    </div>
